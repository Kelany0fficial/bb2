// Combined all JS files into app.js
...
document.addEventListener('click', e => {
  if (e.target.classList.contains('fav-btn')) {
    ...
    // Refresh favorites page if currently on it
...
} else if (document.querySelector('#cart-list')) { // cart.js
  (async () => {
    const settings = await api.load('settings.json');
    ...
    areas.forEach(a => {
      ...
      opt.value = a.id;
      opt.textContent = `${a.name} (${egp(a.fee)})`;
    ...
    function updateCart() {
      ...
      const sub = cartTotal(store.cart);
      const delivery = areas.find(a => a.id === areaSelect.value)?.fee || 0;
      const grand = sub + delivery;

      subTotalEl.textContent = egp(sub);
      deliveryFeeEl.textContent = egp(delivery);
    ...
    areaSelect.addEventListener('change', updateCart);
    ...
    document.querySelector('#whatsapp-order').addEventListener('click', () => {
      ...
      lines.push(`—`);
      lines.push(`الإجمالي: ${egp(cartTotal(store.cart))}`);
      lines.push(`دليفري (${areaSelect.options[areaSelect.selectedIndex].text}): ${egp(areas.find(a => a.id === areaSelect.value).fee)}`);
      lines.push(`*المجموع*: ${egp(cartTotal(store.cart) + areas.find(a => a.id === areaSelect.value).fee)}`);
      lines.push(`—`);
